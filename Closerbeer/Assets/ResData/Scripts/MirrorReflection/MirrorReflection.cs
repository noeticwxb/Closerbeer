...
public class MirrorReflection : MonoBehaviour
{
	...
	public bool	m_EnableOnPhone4	=	false;
	...
	void	Start()
	{
		...
		if( RuntimePlatform.IPhonePlayer == Application.platform )
		{
			...
			//
			if( iPhoneSettings.generation == iPhoneGeneration.iPhone4 && !m_EnableOnPhone4 )
				enabled = false;
		}
#endif
		...
		//
#if !UNITY_EDITOR
		if( !enabled && null != renderer && null != renderer.sharedMaterials )
		{
    ...
    private void CreateMirrorObjects( Camera currentCamera, out Camera reflectionCamera )
    {
        ...
        {
			if( !Mathf.IsPowerOfTwo( m_TextureSize ) )
				m_TextureSize = Mathf.NextPowerOfTwo( m_TextureSize );
        ...
        if( !reflectionCamera ) // catch both not-in-dictionary and in-dictionary-but-deleted-GO
        {
			...
			reflectionCamera.cullingMask = ~((1 << LayerMask.NameToLayer("NoReflect")) & (-1));
